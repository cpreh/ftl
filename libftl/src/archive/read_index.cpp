...
using int_binding = alda::bindings::unsigned_<int_type, std::endian::little>;

using int_vector = std::vector<int_type>;

using offset_binding = alda::bindings::dynamic_len<int_vector, int_binding, int_binding>;

using char_binding = alda::bindings::fundamental<char>;

using string_binding = alda::bindings::dynamic_len<std::string, char_binding, int_binding>;

FCPPT_RECORD_MAKE_LABEL(size_label);

FCPPT_RECORD_MAKE_LABEL(path_label);

using record_binding = alda::bindings::record_variadic<
    fcppt::record::element<size_label, int_binding>,
    fcppt::record::element<path_label, string_binding>>;
...
read_entry(std::istream &_stream, int_type const _offset)
{
  _stream.seekg(fcppt::cast::to_signed(_offset), std::ios_base::beg);

  return fcppt::either::map(
      alda::raw::make_generic<alda::raw::stream::istream, record_binding>(_stream),
      [_offset](alda::raw::element_type<record_binding> &&_result)
      { return transform_entry(_offset, std::move(_result)); });
}

fcppt::either::object<alda::raw::stream::error, libftl::archive::index>
  ...
  return fcppt::either::map(
      fcppt::either::sequence<
          ...
          fcppt::algorithm::map_optional<
              ...
              _offsets,
              [&_stream](int_type const _offset)
              {
                return fcppt::optional::make_if(
                    _offset != fcppt::literal<int_type>(0),
                    [&_stream, _offset] { return read_entry(_stream, _offset); });
              })),
      [](std::vector<index_pair> &&_indices)
      {
        return fcppt::algorithm::map<libftl::archive::index>(
            std::move(_indices),
            [](
                // TODO(philipp): Why does && not work here?
                index_pair _index) {
              return libftl::archive::index::value_type{std::move(_index.first), _index.second};
            });
      });
}

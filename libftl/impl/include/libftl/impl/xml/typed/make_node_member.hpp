...
[[nodiscard]] inline libftl::impl::xml::typed::
    node_member<Required, std::remove_cvref_t<Attributes>, std::remove_cvref_t<Content>>
    make_node_member(std::string &&_name, Attributes &&_attributes, Content &&_content)
{
  return libftl::impl::xml::typed::
      node_member<Required, std::remove_cvref_t<Attributes>, std::remove_cvref_t<Content>>{
          std::move(_name), std::forward<Attributes>(_attributes), std::forward<Content>(_content)};
}
}
#endif

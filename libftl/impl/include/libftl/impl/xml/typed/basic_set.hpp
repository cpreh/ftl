#ifndef LIBFTL_IMPL_XML_TYPED_BASIC_SET_HPP_INCLUDED
#define LIBFTL_IMPL_XML_TYPED_BASIC_SET_HPP_INCLUDED
...
{
template <typename Parsers, typename Arg>
class basic_set
{
public:
  ...
  using map = std::unordered_map<std::string, fcppt::reference<Arg const>>;

  explicit basic_set(Parsers &&_parsers) : parsers_{std::move(_parsers)} {}

  [[nodiscard]] fcppt::either::object<libftl::error, result_type>
  parse(std::vector<Arg> const &_args) const
  {
    return fcppt::either::bind(
        basic_set::make_map(_args), [this](map const &_map) { return this->run_parsers(_map); });
  }

private:
  [[nodiscard]] static fcppt::either::object<libftl::error, map>
  make_map(std::vector<Arg> const &_args)
  {
    return fcppt::algorithm::fold(
        _args,
        fcppt::either::make_success<libftl::error>(map{}),
        [](Arg const &_arg, fcppt::either::object<libftl::error, map> &&_map)
        {
          return fcppt::either::bind(
              std::move(_map),
              [&_arg](map &&_inner_map)
      ...
      Label const &) const
  {
    ...
    return fcppt::optional::maybe(
        ...
        [_used, &name, this](fcppt::reference<fcppt::reference<Arg const> const> const _mapped)
